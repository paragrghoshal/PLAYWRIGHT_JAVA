...
let projectSpecificDetails = {
    jiraProjectBaseURl:"https://paragrghoshal.atlassian.net",
    xrayBaseUrl: "https://xray.cloud.getxray.app/api/v2",
    jiraProjectId: "10006",
    testExecutionIssueTypeId: "10014",
    reportFileName: "./target/surefire-reports/testng-results.xml",
    executionReportFolderPath: "./reports"
};
export default class JiraXrayTestManagement {

    username = process.env.ATLASSIAN_USERNAME;
    password = process.env.ATLASSIAN_API_KEY;
    baseUrl = projectSpecificDetails.jiraProjectBaseURl;//"https://paragrghoshal.atlassian.net";
    xrayBaseUrl = projectSpecificDetails.xrayBaseUrl;//"https://xray.cloud.getxray.app/api/v2";
    ...
    public async uploadToJira(data: { executionReportType: string, reportFileName: string, jiraProjectId: string, testExecutionIssueTypeId: string, issueSummary: string, issueDesc: string, executionReportFolderPath: string }) {
        let issueDetailsObject = await this.importExecutionReportToJiraXray(data.executionReportType, data.reportFileName, data.jiraProjectId, data.testExecutionIssueTypeId, data.issueSummary, data.issueDesc);
        let zipFileName = await this.zipFolder(data.executionReportFolderPath);
        await this.attachFileWithIssue(issueDetailsObject?.testExecutionKey, zipFileName!);
    }
}
let jxtm = new JiraXrayTestManagement();
const paramData = {
    executionReportType: "testng",
    reportFileName: projectSpecificDetails.reportFileName,//"./target/surefire-reports/testng-results.xml",
    jiraProjectId: projectSpecificDetails.jiraProjectId,//"10006",
    testExecutionIssueTypeId: projectSpecificDetails.testExecutionIssueTypeId,//"10014",
    issueSummary: "This script created via automation script",
    issueDesc: "issue desc",
    executionReportFolderPath: projectSpecificDetails.executionReportFolderPath//"./reports"
}
jxtm.uploadToJira(paramData);